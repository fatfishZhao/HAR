...
def get_test_dataset(personid=None, transforms = None, window_size = 256, overlap = 0.5):
    assert personid is not None
    data_root = '/data3/zyx/project/HAR/data'
    meta_df = pd.DataFrame(columns=['motion', 'personid', 'metaid'])
    motions = [];    personids = [];    metaids = []
    for key in motion_dict.keys():
        ...
        for each_person in tmp_personmap.keys():
            if each_person == personid:
                ...
                personids += [each_person] * len(tmp_personmap[each_person])
                metaids += tmp_personmap[each_person]
    meta_df['motion'] = motions;    meta_df['personid'] = personids;    meta_df['metaid'] = metaids
    test_dataset = dataset(dataroot=data_root, anno_pd=meta_df, transforms=transforms['test'], window_size=window_size, overlap=overlap)
    logging.info('%s' % (dt()) + 'test data size,' + str(len(test_dataset)))
    return  test_dataset
